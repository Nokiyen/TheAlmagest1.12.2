	...
	private void renderStars(BufferBuilder renderer, boolean vbo) {
		...
		for(int i=1; i<=88; i++) {
			...
			for(StarData each: targetStars) {
				...
				Vector3d vector1 = this.calcRotate(this.createVector(radius, size, size), each.getCalculatedLong(), -1.0D*each.getCalculatedLat());
				Vector3d vector2 = this.calcRotate(this.createVector(radius, size, -1D*size), each.getCalculatedLong(), -1.0D*each.getCalculatedLat());
				Vector3d vector3 = this.calcRotate(this.createVector(radius, -1D*size, -1D*size), each.getCalculatedLong(), -1.0D*each.getCalculatedLat());
				Vector3d vector4 = this.calcRotate(this.createVector(radius, -1D*size, 1D*size), each.getCalculatedLong(), -1.0D*each.getCalculatedLat());
/*				Vector3d vector1 = this.calcRotate(this.createVector(radius, 3D, 3D), each.getCalculatedLong(), 0);
			...
			for(OtherStarData each: targetOtherStars) {
				double size = Math.max(1.0D - 0.1D*each.magnitude*1.5D, 0.1D);
				Vector3d vector1 = this.calcRotate(this.createVector(radius, size, size), each.ra, -1.0D*each.dec);
				Vector3d vector2 = this.calcRotate(this.createVector(radius, size, -1D*size), each.ra, -1.0D*each.dec);
				Vector3d vector3 = this.calcRotate(this.createVector(radius, -1D*size, -1D*size), each.ra, -1.0D*each.dec);
				Vector3d vector4 = this.calcRotate(this.createVector(radius, -1D*size, 1D*size), each.ra, -1.0D*each.dec);
				renderer.pos(vector1.y, vector1.z, vector1.x).endVertex();
				renderer.pos(vector2.y, vector2.z, vector2.x).endVertex();
				renderer.pos(vector3.y, vector3.z, vector3.x).endVertex();
				renderer.pos(vector4.y, vector4.z, vector4.x).endVertex();
