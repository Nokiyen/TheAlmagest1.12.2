...
public class EventToolTip {
	...
	public void onItemTooltip(ItemTooltipEvent event) {
		...
		//keep abilities.
		HashMap<Integer, ArrayList<Integer>> abilities = StarAbilityCreator.getAbility2(stack);
		...
		tooltip.add(getLine("almagest.gui.tooltip.memory", memory));
		...
		for(Map.Entry<Integer, ArrayList<Integer>> eachAbilitiy: abilities.entrySet()) {
			for(Integer level: eachAbilitiy.getValue()) {
				event.getToolTip().add(getLine("almagest.ability."+eachAbilitiy.getKey()+"."+level+".name"));
			}
		}
